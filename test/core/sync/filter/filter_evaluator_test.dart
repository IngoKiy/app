...
void main() {
  ...
  group('percent_done', () {
    ...
    test(
      '0.5 >= 0.5',
      () => expect(_match(half, 'percent_done >= 0.5'), isTrue),
    );
    test(
      '0.5 > 0.5',
      () => expect(_match(half, 'percent_done > 0.5'), isFalse),
    );
  ...
  group('Datum inkl. now-Arithmetik und Jahr-1', () {
    ...
    test(
      'start_date < now',
      () => expect(
        _match(_task(startDate: DateTime.utc(2026, 1, 1)), 'start_date < now'),
        isTrue,
      ),
    );
    test(
      'end_date >= now',
      () => expect(
        _match(_task(endDate: DateTime.utc(2027, 1, 1)), 'end_date >= now'),
        isTrue,
      ),
    );
  });
