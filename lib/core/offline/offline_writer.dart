...
/// Ergebnis eines Anhang-Schreibvorgangs für die UI (typisierte Variante von
/// [OfflineWriteResult], weil die Section die konkreten Anhänge braucht).
...
class OfflineWriter {
  ...
  /// Lädt Anhänge hoch (Local-First): Dateien werden SOFORT nach
  /// `<ApplicationSupport>/pending_uploads/<key>/` kopiert (Picker-Tempdateien
  /// überleben nicht), Platzhalter-Zeilen (negative IDs, `localFilePath`)
  /// optimistisch eingefügt und der Rest läuft über den [_execute]-Kern:
  /// online-Erfolg ersetzt die Platzhalter durch die Server-Anhänge (im
  /// [OpExecutor.postNonCreate]) und löscht die Kopien; 4xx rollt zurück;
  /// offline bleibt die attachmentUpload-Op in der Outbox.
  Future<AttachmentWriteResult> uploadAttachments(
    ...
    // 1. Kopien in einen op-eigenen Ordner (Schlüssel = eigene Temp-ID).
    ...
    // 2. Platzhalter vorbereiten (IDs vor applyLocal, damit die UI sie kennt).
    final now = _now;
    ...
    final res = await _execute(
      ...
      applyLocal: () async {
        for (final p in placeholders) {
          await _db.taskAttachmentsDao.upsertLocal(
            TaskAttachmentsCompanion.insert(
              id: Value(p.id),
              taskId: taskId,
              fileJson: jsonEncode(
                TaskAttachmentFileDto.fromDomain(p.file).toJSON(),
              ),
              localFilePath: Value(p.localFilePath),
              rawJson: '{}',
            ),
          );
        }
    ...
    switch (res.status) {
      case OfflineWriteStatus.synced:
        final dtos = (res.body as List).cast<TaskAttachmentDto>();
        return AttachmentUploaded(dtos.map((e) => e.toDomain()).toList());
      case OfflineWriteStatus.queued:
        return AttachmentQueued(placeholders);
      case OfflineWriteStatus.rejected:
        return AttachmentFailed(res.error?.statusCode);
    }
  }
  ...
  Future<void> registerDownloadedFile(
    ...
    return _db.taskAttachmentsDao.upsertFromServer(
  ...
  Future<void> _setAttachmentDeleted(int attachmentId, bool deleted) =>
      (_db.update(_db.taskAttachments)
            ..where((a) => a.remoteId.equals(attachmentId)))
          .write(TaskAttachmentsCompanion(isDeleted: Value(deleted)));
    ...
    switch (op.type) {
      ...
      case PendingOpType.attachmentUpload:
        ...
        if (paths.isNotEmpty) {
          await (_db.delete(
            _db.taskAttachments,
          )..where((a) => a.localFilePath.isIn(paths))).go();
          for (final path in paths) {
            await _storage.deleteFileQuietly(path);
          }
        }
      case PendingOpType.attachmentDelete:
        final attachmentId = (op.payload['attachment_id'] as num?)?.toInt();
        if (attachmentId != null) {
          await _setAttachmentDeleted(attachmentId, false);
        }
